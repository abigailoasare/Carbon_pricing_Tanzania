...
tza_IG <-tza_info %>% 
  ...
  dplyr::mutate(incg_100 = as.numeric(
    rattle::binning(
      hh_exp_0_pc,
      bins = 100,
      method = c("wtd.quantile"),
  ...
  select(hh_id,incg_5,incg_10,incg_100)
...
tza_infras_IG5 <-tza_infras %>% 
  weight_by(hh_weights) %>%
  #as_survey(weights = hh_weights) %>%
  group_by(incg_5) %>%
  summarise(access_ely_bd = sum(access_ely_bd, na.rm = TRUE)/n(),
            access_ely_st = sum(access_ely_st, na.rm = TRUE)/n(),
            access_ely_grid = sum(access_ely_grid, na.rm = TRUE)/n(),
            access_ely_solar = sum(access_ely_solar, na.rm = TRUE)/n(),
            access_ely_other =sum(access_ely_other, na.rm = TRUE)/n(),
            access_sanit_st =sum(access_sanit_st, na.rm = TRUE)/n(),
            access_water_st = sum(access_water_st, na.rm = TRUE)/n()
  )%>% 
  ungroup()
...
tza_cook_IG5 <-tza_infras %>% 
  ...
  ungroup() %>% 
  group_by(incg_5) %>%
  mutate(share = num_households / sum(num_households)) %>% 
  ungroup() %>% 
...
tza_lighting_IG5UR <-tza_infras %>% 
  ...
  ungroup()%>% 
  dplyr::select(-num_households) 
unique(tza_lighting_IG5UR$lighting_Label)
...
cpi_1 <- cpi_0 %>%
  ...
  mutate(inflation_factor =ifelse(Year_0 == 2020,
                                  (1/(Year_2018*Year_2019*Year_2020)),
                                  0))
...
ggplot(exp_ana,
       aes(x = factor(incg_5),
           y = hh_exp_USD_pc)) +
  geom_boxplot(aes(weight = hh_weights),
               width = 0.5) +
  theme_bw() + xlab("Income Group") +
  ...
  geom_hline(
    yintercept = mean_exp_USD,
    color = "darkblue",
    linewidth = 1,
    linetype = "dotted"
   )
  #+
  # geom_magnify(from = c(1, 5, 100, 5000), 
  #              to = c(0.5, 2, 10000, 20000), 
  #              #shape="ellipse",
  #              shadow = TRUE,
  #              linetype='dashed',
  #              colour = "darkblue",
...
hh_corr <- tza_merge %>%
  ...
  dplyr::summarise(
    median_item= wtd.quantile(expenditures, 
                              ...
                              probs = 0.5,
                              na.rm=T),
    ninetynineth_item = wtd.quantile(expenditures, 
                                     weights = hh_pop, 
                                     probs = 0.99,
                                     na.rm=T)
  ) %>%
...
Outlier <- sum(hh_merge_1$Outlier)
...
ggplot(oa1,
  ...
  ggtitle("Outliers per Item over Income Group", "Tanzania") +
...
# We try to gain information on the extent of the outlier.
#How much bigger is the outlier than the actual median 99th percentile
...
hh_merge_2 <- hh_merge_1 %>%
  ...
  mutate(expenditures = ifelse(expenditures > ninetynineth_item,
...
## Sensitivity Check on Outlier Sensitive Binning ------------------------


hh_merge_2_bdn <- hh_merge_2 %>%
  group_by(hh_id) %>%
  mutate(hh_exp_0   = sum(expenditures)) %>%
  ungroup() %>%
...
ggplot(hh_other_4,
  ...
  geom_boxplot(aes(weight = hh_weights), 
               ...
               width = 0.75) + 
  theme_bw()+
  ggtitle("Share of Expenditures on other", "Tanzania") + 
...
hm_0 <- hh_merge %>%
  ...
  # To include other, the comment-out the below code 
...
if((row_gas_gdt - nrow(gas_gdt))!= 0){
  ...
  hh_merge <- bind_rows(hh_merge, gas_gdt)%>%
    arrange(hh_id)
}else{
  hh_merge$GTAP <- recode(hh_merge$GTAP, gas = "gasgdt", gdt = "gasgdt")
}
...
ggplot(hh_merge_ely_2,
  ...
  ylab("Share of expenditures on electricity over all expenditures") +
  xlab("Income Group") +
  coord_cartesian(ylim = c(0, 0.1)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))
...
ggplot(hh_merge_ely_2_5, 
       aes(x = factor(incg_5),
           fill = Urban))+
  geom_boxplot(aes(y = share_ely, 
                   weight = hh_weights),
               outlier.shape = NA, 
               varwidth = TRUE) + theme_bw() +
  xlab("Income Group") + 
  ylab ("")+
  coord_cartesian(ylim = c(0, 0.1))+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
  labs(fill = "")+
  scale_x_discrete(labels = c("Overall", "1", "2", "3", "4", "5")) +
