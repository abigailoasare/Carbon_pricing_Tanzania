...
#  HH Food Consumption Expanded
hh_ja1 <- read_dta(file.path(dir[["tza"]], "hh_sec_ja1.dta"))
...
# gather infor on data -----------------------------------

# str(hh_a)
# 
# names(hh_a)
# 
# attr(hh_a$hh_a01_1, "labels")
...
hh_gen <- hh_a %>%
  # dplyr::left_join(urban, by = "y5_rural")%>%
  dplyr::select(y5_hhid, y5_panelweight, y5_rural) %>%
  dplyr::rename(hh_id = y5_hhid,
                hh_weights = y5_panelweight,
                urban = y5_rural)
...
hh_demo <- hh_b %>%
  dplyr::select(y5_hhid, indidy5, hh_b04) %>%
  dplyr::rename(hh_id = y5_hhid) %>%
  group_by(hh_id) %>%
  mutate(hh_size = n()) %>%
...
hh_edu <- hh_c %>%
  ...
  dplyr::rename(hh_id = y5_hhid, hh_edu = hh_c07)
...
hh_income <- hh_e %>%
  ...
  dplyr::rename(hh_id = y5_hhid, indiv_id = indidy5)
...
hhyr_food <- hh_food %>% 
  ...
  mutate(expenditures=(amount/7)*366) %>% 
...
# Food Expenditure Expanded Non-Standard Units(NSUs) -------------------------

# This is basically the same as that of SEC J1, except that this module 
# included an expanded list of Non-Standard Units (NSUs).
# "Non-standard units" refer to units of measurement that are not part of 
# the commonly used and standardized system of measurement,
# which is known as the International System of Units (SI)
# The number of food items was also expanded to 75 over the last 7 days.

# Extract  item labels   

item_labels <- attr(hh_ja1$itemcode, "labels")

# Create a new dataframe with variable names, variable labels, and value labels
labels_foodexpanded <- data.frame(
  code = unname(item_labels),
  food_item = names(item_labels)
)
...
write.xlsx(labels_foodexpanded ,file.path(dir[["tza_processed"]], "labels_foodexpandedNSU_tza.xlsx"))
...
hh_food_nsu <- hh_ja1 %>%
  ...
  dplyr::rename(
    hh_id = y5_hhid,
    consume_7ut = hh_ja02_1,
    consume_7qqt = hh_ja02_2,
    purchase_7ut = hh_ja03_1,
    purchase_7qqt = hh_ja03_2,
    amount = hh_ja04,
    ownprod_ut = hh_ja05_1,
    ownprod_qqt = hh_ja05_2
  )

hh_food_nsu$hh_id <- gsub("-", "", hh_food_nsu$hh_id)



# Replace missings with zero for futher analyses

hh_food_nsu[is.na(hh_food_nsu)] <- 0

## Select relevant variables And Calculate Yearly Expenditure ---------------
# Amount is the expenditure for the past 7 days
# 2020 was a leap year

hhyr_food_nsu <- hh_food_nsu %>% 
  select(hh_id,itemcode,amount) %>% 
  mutate(expenditures=(amount/7)*366)%>% 
  select(hh_id,itemcode,expenditures)
...
hhyr_nonfood1 <- hh_nonfood1 %>% 
  select(hh_id,itemcode,amount) %>% 
  mutate(expenditures=ifelse(itemcode<=103,((amount/7)*366),amount),
         expenditures=ifelse(itemcode>=201,((amount/30)*366),expenditures))%>% 
  select(hh_id,itemcode,expenditures)
...
tza_clean <-tza_clean %>% 
  pivot_longer(cols=29:140, names_to="itemcode", values_to="expenditures")
